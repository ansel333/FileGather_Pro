...
from PyQt5.QtGui import QIcon, QColor, QPalette, QLinearGradient, QBrush, QFont
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QMenu, QAction, QInputDialog, QTextEdit,
...
class FileGatherPro(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.keyword_entry = QTextEdit()
        self.keyword_entry.setPlaceholderText("输入文件名包含的关键词，支持多行输入")
        self.keyword_entry.setFixedHeight(80)  # 设置一个适合多行输入的高度
    ...
    def start_search(self):
        ...
        all_found_files = {}  # 使用字典以路径为键，避免重复文件
        ...
        if self.cancel_search:
            ...
            if len(keywords) > 1 or any(not files for files in keyword_results.values()):
                 dialog = KeywordSearchResultDialog(self, keyword_results)
                 dialog.exec_()
