...
### 🎨 用户界面
- **针对 1080p 显示屏优化**：窗口 800×750 像素，紧凑高效
- 现代化 PyQt6 界面（6.7.1）
- 白色背景+彩色边框按钮设计
- 单行紧凑按钮布局（8 个按钮，完美居中）
- 即时搜索结果预览，使用树形控件显示
- **增强的关键词显示**：用于已找到和未找到关键词的树形控件
- 右键上下文菜单
- 进度提示和状态反馈
- 条件可见的取消按钮
- **重置按钮**：清除所有数据并恢复初始状态
...
### 🎉 v2.5.1 (2025-12-01) - UI 优化与关键词显示增强
**重大改进**：
- 🎨 **1080p 显示屏紧凑 UI**
  - 窗口尺寸优化：1000×1050 → **800×750 像素**（减少 25%）
  - 完美适配 1080p（1920×1080）笔记本/显示器
  - 窗口尺寸固定，最小高度 650px，保证稳定性
  - 所有组件按比例缩小，同时保持可用性
  
- 🔍 **增强的关键词显示系统**
  - 未找到关键词：可折叠的**树形控件**，具有视觉层次结构
  - 单结果关键词：单独的树形控件，便于识别
  - 所有已找到的关键词现已显示（不仅仅是多结果关键词）
  - 复制反馈：显示关键词计数，3 秒自动清除
  - 更好的视觉组织，未找到关键词使用 #e74c3c（红色）文本
  
- 🎯 **按钮布局优化**
  - 单行居中按钮布局（9 个位置中的 8 个按钮）
  - 固定按钮尺寸：**75×26 像素**，间距 3px
  - 完美对齐，经过数学精度验证
  - 所有按钮一次可见，无需水平滚动
  
- 🐛 **关键缺陷修复**
  - 修复了连续搜索失败（精确查找后进行模糊查找）
  - 改进了搜索操作期间的按钮状态管理
  - 取消按钮在活动搜索期间始终可见
  - **重置按钮**：新增按钮，用于清除所有数据并重置窗口到初始状态
...
- 🧪 **质量保证**
  ...
  - 12 个 PyQt6 特定测试已验证
  - 所有 UI 组件已测试和验证
  - 在 Windows 11 1080p 分辨率下测试
  
**技术细节**：
- 窗口：`setGeometry(100, 100, 800, 750)`，`setMinimumSize(800, 650)`
- 按钮布局：9 个位置，8 个按钮，使用 stretch 布局居中
- 树形控件：最大高度 100px，可折叠，正确的缩进
- 反馈标签：#27ae60（绿色）文本，8pt 字体，3000ms 计时器
- 版本：2.5.1，所有文件已更新
- 发布资产：Windows .exe、macOS（Intel/ARM）、Linux .deb

**相关文件**：
- `components/ui_builder.py` - 窗口几何图形、按钮布局、树形控件
- `components/main_window.py` - 版本处理、窗口初始化
- `components/functions/results_manager.py` - 关键词显示逻辑
- `components/functions/ui_interactions.py` - 重置功能
